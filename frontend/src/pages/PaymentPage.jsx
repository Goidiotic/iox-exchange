...
export default function PaymentPage() {
  ...
  const submitPayment = async () => {
    if (!transactionId.trim()) {
      toast.error('Enter the M3 Wallet transaction ID');
      return;
    }
      ...
      <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="grid aspect-square place-items-center rounded-lg border border-dashed border-cyanx/40 bg-cyanx/10 p-5">
              <div className="text-center">
                ...
                <p className="mt-4 flex items-center justify-center gap-2 text-sm font-semibold text-white">
                  <QrCode size={16} /> Scan with M3 Wallet
                </p>
        ...
        <Card hover={false}>
          <div className="space-y-4">
            ...
            <FormField label="M3 Wallet transaction ID">
              <input
                className="field"
                value={transactionId}
                onChange={(event) => setTransactionId(event.target.value)}
                placeholder="M3 Wallet transaction ID"
              />
            </FormField>
            <div className="panel p-4 text-sm text-slate-300">
              ...
              <p className="mt-2">Pay the exact INR amount from your M3 Wallet to the receiving address, then enter the transaction ID. Your payment will be checked automatically.</p>
            </div>
